...
> JavaScript 原生代码渲染内核 · 零依赖 · 22 类 token 语义体系
...
WordPress、VS Code 等官方集成应放在独立仓库中，默认使用 JSRay Core，并保持完整可用，不以付费锁定基础能力。平台层应开放 renderer adapter / hook，方便宿主项目在需要时接入其它渲染器。
...
没有语言 class 时，`JSRay.detectLanguage()` 可以自动识别常见 PHP、Go、Swift、Kotlin、Dart、Lua、SQL、YAML、HTML、CSS、JavaScript、Python、Shell 等片段。
...
后续会加入 Dracula、GitHub、Solarized 等。切换主题时**只换 theme 的 `<link>`**，`jsray.css` 保持不动。
